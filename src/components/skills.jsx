import {useState} from "react";
import {cn} from "@/lib/utils.js";

const skills = [
    // Languages
    { name: "JavaScript", level: 90, category: "language" },
    { name: "TypeScript", level: 95, category: "language" },
    { name: "Java", level: 95, category: "language" },
    { name: "Python", level: 95, category: "language" },
    { name: "C#", level: 85, category: "language" },
    { name: "C", level: 90, category: "language" },
    { name: "C++", level: 75, category: "language" },

    // Frontend
    { name: "HTML/CSS", level: 95, category: "frontend" },
    { name: "React.js", level: 85, category: "frontend" },
    { name: "Next.js", level: 85, category: "frontend" },
    { name: "Angular.js", level: 75, category: "frontend" },

    // Backend
    { name: "Spring Boot", level: 85, category: "backend" },
    { name: "ASP.NET", level: 80, category: "backend" },
    { name: "Node.js", level: 80, category: "backend" },
    { name: "Express.js", level: 80, category: "backend" },

    // Tools / DevOps / DB
    { name: "PostgreSQL", level: 85, category: "tool" },
    { name: "MongoDB", level: 85, category: "tool" },
    { name: "MySQL", level: 80, category: "tool" },
    { name: "SQL Server", level: 75, category: "tool" },
    { name: "Docker", level: 85, category: "tool" },
    { name: "Jenkins", level: 80, category: "tool" },
    { name: "RabbitMQ", level: 75, category: "tool" },
    { name: "Kafka", level: 70, category: "tool" },
    { name: "AWS", level: 70, category: "tool" },
    { name: "GCP", level: 70, category: "tool" },
    { name: "OpenStack", level: 65, category: "tool" },
    { name: "Git", level: 95, category: "tool" },
];

const categories = ["All", "Language", "Frontend", "Backend", "Tool"];

export const SkillSection = () => {
    const [activeCategory, setActiveCategory] = useState("All");
    const filteredSkills = skills.filter(skill => activeCategory === "All" || skill.category.toLowerCase() === activeCategory.toLowerCase());

    return (
        <section id="skills" className="py-24 px-4 relative">
            <div className="container mx-auto max-w-5xl">
                <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center">
                    My <span className="text-primary">Skills</span>
                </h2>
                <div className="flex flex-wrap justify-center gap-4 mb-12">
                    {categories.map((category, key) => (
                        <button onClick={() => setActiveCategory(category)} key={key}
                            className={cn(
                                "px-5 py-2 rounded-full transition-colors duration-300 capitalize",
                                activeCategory === category
                                    ? "bg-primary text-primary-foreground"
                                    : "bg-secondary/70 text-foreground hover:bg-secondary/80"
                            )}
                        >{category}</button>
                    ))}
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredSkills.map((skill, key) => (
                        <div
                            key={key}
                            className="bg-card p-6 rounded-lg shadow-xs card-hover"
                        >
                            <div className="text-left mb-4">
                                <h3 className="font-semibold text-lg">{skill.name}</h3>
                            </div>
                            <div className="w-full bg-secondary/50 h-2 rounded-full overflow-hidden">
                                <div
                                    className="bg-primary h-2 rounded-full origin-left animate-[grow_1.5s_ease-out]"
                                    style={{width: skill.level + "%"}}
                                />
                            </div>
                            <div className="text-right mt-1">
                              <span className="text-sm text-muted-foreground">
                                {skill.level}%
                              </span>
                            </div>
                        </div>
                    ))}
                </div>
            </div>

        </section>
    )
}
